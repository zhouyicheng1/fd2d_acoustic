...
!force
  ...
  do n=1,num_force
    ...
    if (SrcSmooth .eqv. .true.) then
      ...
      call cal_norm_delt(normdx, x0-0.5,z0,real(2/2.0,SP), real(2/2.0,SP) )
      call cal_norm_delt(normdz, x0,z0-0.5,real(2/2.0,SP), real(2/2.0,SP) )
      if (freenode .and. sk+2>nk2)  then
        normdx=normdx/sum(normdx(:,-2:nk2-sk))
        normdz=normdz/sum(normdz(:,-2:nk2-sk))
      end if
    end if
...
subroutine cal_norm_delt(delt,x0,z0,rx0,rz0)
  real(SP),dimension(-2:2,-2:2),intent(out) :: delt
