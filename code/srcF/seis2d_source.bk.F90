...
integer :: fxid,fzid,mxxid,mzzid,mxzid,vxid,vzid,resrc_pointid
...
subroutine read_src_para(fnm_src_conf,fnm_conf)
  ...
  !resrc
  call string_conf(fid,1,"number_of_resrc_source",2,num_resrc)
  ...
  do n=1,num_resrc
      ...
      k=0
       ...
       call string_conf(fid1,1,'SOURCE_ROOT',2,pnm_src1)       ! pnm_src--SOURCE_ROOT = ./input
       fnm1=trim(pnm_src1)//'/'//'seismo'//'_'//set_mpi_subfix(n_i,n_k)//'_reverse'//'.nc'
       call nfseis_varget(fnm1,'Vx_reverse',resrc_Vx(:,n),(/k,1/),(/1,ntt/),(/1,1/))
       call nfseis_varget(fnm1,'Vz_reverse',resrc_Vz(:,n),(/k,1/),(/1,ntt/),(/1,1/))
       k=k+1
       A(k1,k2) = k
       close(fid1)
  end do
...
subroutine srcnode_skel(filenm,nfrc,ntwfrc,nmom,ntwmom)
  ...
  if (nresrc>0) then
    ...
    ! -- define variable
    ierr=nf90_def_var(ncid,'Vx',SEISNC_DATATYPE,(/ntwmomid,nresrcid/),vid)
    ierr=nf90_def_var(ncid,'Vz',SEISNC_DATATYPE,(/ntwmomid,nresrcid/),vid)
