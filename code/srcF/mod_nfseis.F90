...
interface nfseis_put
  ...
  integer,parameter,public :: SEISNC_DATATYPE=NF90_DOUBLE
#else
  integer,parameter,public :: SEISNC_DATATYPE=NF90_FLOAT
#endif
...
subroutine nfseis_snap_def(filenm,ncid,tid,stept,subc,title)
  ...
  ierr=nf90_def_dim(ncid, 'K', subc(2), zdimid)
...
subroutine nfseis_snap_defvar(ncid,varnm,vid)
  ...
  integer,intent(in ) :: ncid
  integer,intent(out) :: vid
  integer ierr,tdimid,xdimid,zdimid
  ierr=nf90_inq_dimid(ncid,'time',tdimid)
...
!*************************************************************************
!*                    2D grid point value file                           *
!*************************************************************************
subroutine nfseis_grid2d_def(filenm,nx,nz,ncid,title)
  character (len=*),intent(in) :: filenm
  ...
  ierr=nf90_create( path=trim(filenm), cmode= nf90_clobber, ncid = ncid )
  call nfseis_except(ierr,'grid2d_def:'//trim(filenm))
  ierr=nf90_set_fill(ncid,nf90_nofill,oldMode)
  call nfseis_except(ierr,'set_fill in grid2d_def')
  ! -- define dim
  ierr=nf90_def_dim(ncid, 'I', nx, Iid)
  call nfseis_except(ierr,'I dim in grid2d_def')
  ierr=nf90_def_dim(ncid, 'K', nz, Kid)
  call nfseis_except(ierr,'K dim in grid2d_def')
  !-- define global attribute
  if (present(title) ) then
    ierr=nf90_put_att(ncid,NF90_GLOBAL,"title",title )
    call nfseis_except(ierr,'title att in grid2d_def')
  end if
end subroutine nfseis_grid2d_def
subroutine nfseis_grid2d_defvar(ncid,varnm,vid)
  integer,intent(in) :: ncid
  character (len=*),intent(in) :: varnm
  integer,intent(out) :: vid
  integer :: ierr,Iid,Kid
  !--
  ierr=nf90_inq_dimid(ncid,'I',Iid)
  call nfseis_except(ierr,'I dim in grid2d_addvar')
  ierr=nf90_inq_dimid(ncid,'K',Kid)
  call nfseis_except(ierr,'K dim in grid2d_addvar')
  ierr=nf90_def_var(ncid,varnm,SEISNC_DATATYPE, &
    (/ Iid,Kid /), vid )
  call nfseis_except(ierr,'add var in grid2d_addvar')
end subroutine nfseis_grid2d_defvar
subroutine nfseis_grid2d_enddef(ncid)
  integer ncid
  integer ierr
  ierr=nf90_enddef(ncid)
  call nfseis_except(ierr,'enddef in grid2d_enddef')
end subroutine nfseis_grid2d_enddef

subroutine nfseis_grid2d_skel(filenm,nx,nz,title)
  character (len=*),intent(in) :: filenm
  integer,intent(in) :: nx,nz
  character (len=*),intent(in),optional :: title
  ...
  ierr=nf90_create( path=trim(filenm), cmode= nf90_clobber, ncid = ncid )
  call nfseis_except(ierr,'grid2d_header:'//trim(filenm))
  ierr=nf90_set_fill(ncid,nf90_nofill,oldMode)
  call nfseis_except(ierr,'set_fill in grid2d_header')
  ! -- define dim
  ierr=nf90_def_dim(ncid, 'I', nx, Iid)
  call nfseis_except(ierr,'I dim in grid2d_header')
  ierr=nf90_def_dim(ncid, 'K', nz, Kid)
  call nfseis_except(ierr,'K dim in grid2d_header')
  !-- define global attribute
  if (present(title) ) then
    ierr=nf90_put_att(ncid,NF90_GLOBAL,"title",title )
    call nfseis_except(ierr,'title att in grid2d_header')
  end if
  ierr=nf90_enddef(ncid)
  call nfseis_except(ierr,'endedf in grid2d_header')
  ierr=nf90_close(ncid)
  call nfseis_except(ierr,'file close in grid2d_header')
end subroutine nfseis_grid2d_skel

subroutine nfseis_grid2d_addvar(filenm,aVarNm)
  ...
  call nfseis_except(ierr,'grid2d_addvar:'//trim(filenm))
  ierr=nf90_redef( ncid)
  call nfseis_except(ierr,'redef in grid2d_addvar')
  ierr=nf90_set_fill(ncid,nf90_nofill,oldMode)
  call nfseis_except(ierr,'set_fill in grid2d_addvar')
  ! -- req dim
  ierr=nf90_inq_dimid(ncid,'I',Iid)
  call nfseis_except(ierr,'I dim in grid2d_addvar')
  ierr=nf90_inq_dimid(ncid,'K',Kid)
  call nfseis_except(ierr,'K dim in grid2d_addvar')
  ! -- define variable
  ierr=nf90_def_var(ncid, trim(aVarNm), SEISNC_DATATYPE,(/ Iid,Kid /),varid)
  call nfseis_except(ierr,'var def in grid2d_addvar')
  ierr=nf90_enddef(ncid)
  call nfseis_except(ierr,'enddef in grid2d_addvar')
  ierr=nf90_close(ncid)
  call nfseis_except(ierr,'file close in grid2d_addvar')
end subroutine nfseis_grid2d_addvar

subroutine nfseis_grid2d_attput(filenm,         &
    subs,subc,subt,                    &
  ...
  !--
  ierr=nf90_close(ncid)
end subroutine nfseis_grid2d_attput
...
subroutine nfseis_handle_err(ierr)
  ...
  if (ierr /= nf90_noerr) then
    print *, trim(nf90_strerror(ierr))
    print *, "Stopped caused by ncfile"
    stop 2
  end if
...
! vim:ft=fortran:ts=2:sw=2:nu:et:
