  ...
  do n=1,num_snap
    ...
    if (ResultOnColocatedGrid .eqv. .true.) then
      ...
      do k=subs(2),sube(2),subt(2)
        ...
        do i=subs(1),sube(1),subt(1)
          si=si+1
          p1(1)=(Vx(i,k)+Vx(i-1,k))/2.0
          ierr=nf90_put_var(vel_ncid(n),vel_vid(1,n), p1,   &
            (/si,sk,n1+1/),(/1,1,1/),(/1,1,1/) )
          p1(1)=(Vz(i,k)+Vz(i,k-1))/2.0
          ierr=nf90_put_var(vel_ncid(n),vel_vid(2,n), p1,   &
            (/si,sk,n1+1/),(/1,1,1/),(/1,1,1/) )
        end do
      end do
#endif
